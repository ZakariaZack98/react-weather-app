...
const Chart = ({ data, activeMode }) => {
  return (
    <div className="chartSec p-4 rounded-xl surface-card-darkest w-full h-fit -translate-y-4 relative">
      ...
      <div className="iconLine h-[10%] flex justify-between items-center md:px-6 px-2 border border-[rgba(255,255,255,0.14)] pb-2 pt-4 rounded-t-xl">
        {
            ...
            <div key={hourlyData.dt} className='flex flex-col justify-center items-center'>
              {
                activeMode === 'Wind' ? (<WindDirectionIcon size={25} deg={hourlyData?.wind?.deg} />) : (
                  <picture>
                    <img src={`https://openweathermap.org/img/wn/${hourlyData?.weather[0]?.icon}@2x.png`} alt="" className='2xl:h-10 lg:h-8 md:h-5 h-4 scale-200' />
                  </picture>
                )
              }
              {
                activeMode === 'Wind' ? <p className='text-sm pt-3'>{hourlyData?.wind?.speed}km/h</p> : <p className='md:text-sm text-xs'>{Math.round(hourlyData.main.temp)}°c</p>
              }
            </div>
          ))
        }
      </div>
      <div className="w-full 2xl:h-[45dvh] lg:h-100 h-[50dvh]  lg:px-5 px-0 border border-[rgba(255,255,255,0.14)] p-2 rounded-b-xl">
        {
